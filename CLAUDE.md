...
- **HOW to gloss — FOOTNOTES, never inline parentheses (LEARNER PREFERENCE, authoritative, from session 13).** Inline parentheticals **break reading flow**, and the learner **reads these texts aloud**, so a mid-sentence gloss derails both comprehension and pronunciation practice. Therefore, in **`03-reading` passages, the `03-reading` conversations, and `04-listening` transcripts/prompts**:
  - Mark the glossed item with a **small superscript number** immediately after it — **¹ ² ³ ⁴ ⁵ ⁶ ⁷ ⁸ ⁹ ¹⁰ ¹¹ …** (Unicode superscripts, which render everywhere). Nothing else interrupts the line.
  - Collect **all** glosses in a single **`### 📖 Notas`** block **at the very end of the file**, after the text (and after the comprehension questions in a graded reading, so the questions stay adjacent to the passage). One numbered line per gloss: `¹ **wort** = traducción · breve nota si hace falta`.
  - **The running text stays clean German.** No `*(= …)*`, no `«…»` explanations inside sentences.
  - This is about **presentation only** — the glossing *policy* is unchanged: still gloss above-level structures briefly, still don't drill them.
  - Applies to **worked examples in `02-grammar`** too when they'd otherwise carry a mid-sentence aside; short teaching tables and rule explanations are exempt (they are not continuous reading).
  - *(If the learner ever asks for clickable notes, GitHub-style markdown footnotes `[^1]` / `[^1]: …` are the drop-in alternative — same structure, renderer-dependent.)*
...
- **Per-session rotation (the load rule).** A fixed **spine runs every session**: `00-check` (the graded retrieval warm-up that opens the session), `01-vocab`, `02-grammar`, and `04-listening`. **Slot `03` holds reading + conversations + writing every session (LEARNER PREFERENCE, authoritative — updated session 12).** In the same session it runs:
  - **(a) `03-writing` — 2 tasks, the PRIORITY** (substantial output reps; writing is the lagging, most-wanted skill).
  - **(b) `03-reading` — 1 GRADED reading, denser (~10–12 comprehension questions)** — no longer "compact" (supersedes the old ~6–8).
  - **(c) `03-reading` CONVERSATIONS — 3 LONG, read-only dialogues** — **input only: NO questions, NOT graded, no `progress/scores.csv` row, no `/grade` review** (self-directed input, like `04-listening`). They show the **current theme + the grammar taught so far** in natural use, and they are the vocabulary **recycling engine** — they must recycle the session's new vocab (**coverage rule, §10**) plus a rotating selection of the theme's earlier vocab and some spaced older items. They double as **pronunciation practice** (the learner reads them aloud). May carry a little i+1 grammar, glossed. **Files:** graded reading = `03-reading/exercise-01.md` (+ fillable `answers-01.md`); conversations = `03-reading/conversation-01.md … conversation-03.md` (**no** `answers`/`review` files).
  This makes sessions **long (~2–2.5 h)** — **accepted**: the learner prefers depth and will **split a session across two days** if needed (**time is not the constraint; quality is**). See §8, §6, §10 and memories `[[writing-every-session-output-practice]]`, `[[wants-longer-denser-exercises]]`, `[[more-input-conversations-slower-themes]]`. **Grammar still advances the roadmap every session** and stays the backbone. Listening is the **short staple (~10–15 min)**. **Exception — test sessions (7th/14th):** slot `03` yields entirely to the `05-test` (skip the graded reading, the conversations, and the writing — the test carries production/comprehension).
...
- **Verbs — TWO ROWS for irregulars (LEARNER PREFERENCE, authoritative, from session 12).** The old single-row chain (`sprechen – sprach – hat gesprochen` as the headword) is **superseded**: it printed the past forms on the **front** of the Anki card, so the learner only ever *recognized* them, never *produced* them — the opposite of the testing effect this kit is built on. Instead:
  - **Row A — meaning card:** headword = the plain **infinitive** (`sprechen`); translation = Spanish; example in the present. *(Separables keep their pipe: `einkaufen (ein|kaufen)`.)*
  - **Row B — forms card (ONLY when needed):** headword = **`<infinitive> (Prät. + Part. II)`** (e.g. `sprechen (Prät. + Part. II)`); the **translation field carries the ANSWER** = **`Präteritum – auxiliary + Partizip II`** (e.g. `sprach – hat gesprochen`); the **IPA column holds the PARTIZIP II's** pronunciation (that is the form the learner says); the example sentence is in the **Perfekt** (`Ich habe mit Anna gesprochen.`). So the infinitive is on the front and the learner must **recall** the forms.
  - **When Row B is required:** the verb is **strong/irregular** (`sprechen, nehmen, gehen`) **or** takes the auxiliary **`sein`** (even if weak: `reisen – ist gereist`) — because the auxiliary is unpredictable. **Weak verbs with `haben` get NO forms card** (`kaufen → hat gekauft` is derivable from the rule) — this keeps the Anki review load honest.
  - **Priority note:** what is *drilled and corrected now* is the **Partizip II + auxiliary** (the Perfekt is the spoken past, roadmap A1 #16–17); the **Präteritum** rides along on the same card for recognition and pays off later for written narration (A2 #7 / B1 #6). Keeping both avoids re-cutting the cards later.
  - The two rows are **separate dedup keys** (`sprechen` ≠ `sprechen (Prät. + Part. II)`), so both coexist in the ledger and in Anki without colliding.
...
- A theme drives **two things only**: (a) the session's new **vocabulary** (the bulk of it, from the theme's scenarios and key vocab domains) plus some high-frequency general words; and (b) the **context/topic** of `03-reading` (the graded reading **and** the 3 conversations), `03-writing`, `04-listening`, and live speaking.
...
- The current theme is **advanced adaptively by `/close-day`** (guardrails: stay **at least ~3** sessions, **at most ~10**). **LEARNER PREFERENCE (session 12): prefer ~4–5 sessions per theme** — do **NOT** advance at the 3-session floor; stay until the theme's vocabulary is genuinely **exhausted / recycled** (seen in context multiple times), not merely introduced, so words get spaced repetition instead of a single exposure. The **conversations (§2/§10) are the recycling engine** that makes a ~4–5-session theme pay off. Advance before ~4 only if the theme is clearly mastered; the ~10 ceiling stays as the hard stop. (This was the learner's observation that themes were advancing too fast to use all the vocab — see memory `[[more-input-conversations-slower-themes]]`.)

---

## 7. Graded vs. self-reported skills (authoritative)

- **Objectively graded:** **grammar, the graded reading exercise** (`03-reading/exercise-01.md` + its comprehension questions), **`00-check`, and writing** (writing holistically via `/grade-writing`). These produce **`progress/scores.csv` rows** and **`/grade` review files** (`review-NN.md` / `review.md`). `/lesson`'s `scores.csv`-driven adaptive difficulty applies to these segments only.
- **Ungraded input inside `03-reading`:** the **3 read-only conversations** (§2, `conversation-01…03.md`) are **input only** — **no questions, no `progress/scores.csv` row, no `/grade` review** (self-directed input, exactly like `04-listening`). Only the **graded reading exercise** above is scored. (New words the learner meets in the conversations still flow to the ledger / `/harvest`.)
...
- **Format by layer:**
  ...
  - `03-reading` (the **graded exercise** only) / `04-listening` → **comprehension mix:** MC/TF for gist, open short-answer for detail. The **3 read-only conversations** in `03-reading` carry **no items** (input only, ungraded — see §2/§7).
  ...
  - `05-test` at the **14th** session (90 min) → **exam-like:** sections per skill, multi-format, mirroring a real proficiency test.
- **Test & recalibration cadence (LEARNER PREFERENCE, authoritative — the SINGLE SOURCE for cadence).** All indexed by session number `N`: a **15-minute consolidation test every 7th session**, and a **full ~90-minute exam-like test every 14th session** (`05-test`). This was **shortened from every 28th → every 14th at the learner's request (session 10)**, to get proficiency feedback / recalibration input roughly twice as often. **`/recalibrate` runs on the SAME 14-session cadence**, aligned with — and reading — that 90-minute test. Because **14 is a multiple of 7**, a 14th session runs the 90-minute test and it **takes precedence** (the every-7th 15-minute rule does not also fire that session). This is **separate from `LEARNING-PACE.md` regeneration**, which is a lighter, *more*-frequent throughput cadence — session 1 + **every 3rd session** — and is deliberately **left unchanged** (it already gives faster tactical input than 14). See memory `[[faster-assessment-cadence]]`.
- **Volume / length — denser than the kit's default (LEARNER PREFERENCE, authoritative).** The learner is fast and finds thin sets too short ("es muy poco"); **make graded segments substantially longer** and use more of the 1.5 h. Per-session targets: **`02-grammar` ≥ ~30 items across 2–3 exercises**; **`03-reading` a longer GRADED passage + ~10–12 questions**, **plus 3 LONG read-only CONVERSATIONS** (input, ungraded — the vocab **recycling engine**, see §2/§10); **`03-writing` 2 tasks, fuller (~5–8 sentences each)**; **7th-session `05-test` ~25–30 items**. Keep **`00-check` fast** (its job is speed) and **`04-listening` short** (~10–15 min staple). **Vocab cap unchanged** (~15–30; gated by the learner's pronunciation-perfection time). Scales further via `LEARNING-PACE.md` multipliers. **More *real* production, never filler/repetition.** See memory `[[wants-longer-denser-exercises]]`.
...
**Coverage rule (LEARNER PREFERENCE, session 12):** ~20 new words can't be consolidated by a single reading, so the session's **3 read-only conversations (§2) must recycle the new vocab such that EVERY new item appears at least once in context**, plus a rotating selection of the **theme's earlier vocab** and some **spaced older items** — the conversations are the vocabulary **recycling engine**. This (with the ~4–5-session theme pace, §6) is what makes the vocab actually *used*, not just introduced. If even so a batch feels unconsolidated, flex the cap down toward ~15. See memory `[[more-input-conversations-slower-themes]]`.
