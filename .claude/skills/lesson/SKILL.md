...
description: >
  ...
  check every session, a 15-minute consolidation test every 7th session, and a 90-minute test every 28th
...
- `05-test`, 28th session (90 min) → **exam-like:** sections per skill, multi-format.
...
7. **05-test (cadence by `N`, explicit durations).** **Precedence:** when `N` is a multiple of 28, the **90-minute standalone test TAKES PRECEDENCE** over the every-7th-session 15-minute test — the 28th is test-only and the 15-minute rule does **not** also apply that session (28 is itself a multiple of 7, so check the 28th case first).
   - every 7th session (that is **not** also a 28th) → a **15-minute** consolidation test to `lessons/session-NNN/class/05-test/test.md`; a **balanced mix, production-heavy** (per *Assessment item formats*); make the taught content lighter so the total stays within the session length.
...
- on cadence (7th / 28th session), `lessons/session-NNN/class/05-test/test.md` plus the fillable worksheet `05-test/answers.md`.
